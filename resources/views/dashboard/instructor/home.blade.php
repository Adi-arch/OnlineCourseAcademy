            ...
            <div class="col-md-6 offset-md-3" style="margin-top: 45px">
                 ...
                 <table class="table table-striped table-inverse table-responsive">
                     <thead class="thead-inverse">
                         ...
                         <tbody>
                             <tr>
                                 <td>{{Auth::guard('instructor')->user()->name }}</td>
                                 <td>{{Auth::guard('instructor')->user()->email }}</td>
                                 <td>{{Auth::guard('instructor')->user()->qualification }}</td>
                                 
                                 <td>
                                    <a href="{{ route('instructor.logout') }}" onclick="event.preventDefault();document.getElementById('logout-form').submit();">Logout</a>
                                    <form action="{{ route('instructor.logout') }}" method="post" class="d-none" id="logout-form">@csrf</form>
                                 </td>
                             </tr>
                         </tbody>
                 </table>
